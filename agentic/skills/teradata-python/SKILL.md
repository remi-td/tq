...
def verify_pipeline(cursor, prefix: str):
    # Check row counts
    for tbl in ["stg_customers", "stg_parts", "stg_orders", "stg_lineitem", "fct_order_lineitem"]:
        cursor.execute(f"SELECT COUNT(*) FROM {prefix}{tbl}")
        cnt = cursor.fetchone()[0]
        print(f"Table {prefix}{tbl}: {cnt:,} rows")

    # Check metric totals
    cursor.execute(f"SELECT SUM(net_revenue), SUM(profit) FROM {prefix}fct_order_lineitem")
    row = cursor.fetchone()
    print(f"Fact Total Net Revenue: ${row[0]:,.2f}, Total Profit: ${row[1]:,.2f}")
```

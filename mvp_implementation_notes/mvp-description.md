...
- `query <QUERY>` - Execute a SQL query

Global Options:
- `-l, --logon` - Database logon string format: "user:password@databaseserver:port/schema"
  - **Optional**: If not provided, reads from `TQ_LOGON` environment variable
...
- `--driver-lib-dir` - Directory containing Teradata driver library
- `-h, --help` - Print help
...
# Examples:
cargo run -- -l "user:pass@host:1025/db" query "SELECT * FROM table"
cargo run -- -l "user:pass@host:1025/db" ping
TQ_LOGON="user:pass@host:1025/db" cargo run -- query "SELECT 1 AS col"
cargo run -- query --format json "SELECT 1 AS col"
```
...
- `TQ_LOGON` - **Primary**: Complete connection string "user:password@host:port/database"
  - If set, `--logon` parameter becomes optional
  - Example: `TQ_LOGON="myuser:mypass@myhost:1025/mydb"`
